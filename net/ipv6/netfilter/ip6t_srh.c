...
static bool srh_mt6(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	int hdrlen, nsidoff, psidoff, srhoff = 0;
	struct in6_addr *psid, *nsid;
	struct in6_addr _psid, _nsid;
	...
	if (srhinfo->mt_flags & IP6T_SRH_PSID) {
		...
		if (NF_SRH_INVF(srhinfo, IP6T_SRH_INV_PSID,
				ipv6_masked_addr_cmp(psid, &srhinfo->pmsk,
						     &srhinfo->psid)))
			return false;
	}
	/**
	 * SRH next SID matching
	 */
	if (srhinfo->mt_flags & IP6T_SRH_NSID) {
		...
		if (NF_SRH_INVF(srhinfo, IP6T_SRH_INV_NSID,
				ipv6_masked_addr_cmp(nsid, &srhinfo->nmsk,
						     &srhinfo->nsid)))
			return false;
	}
	return true;
